...
## 当前结论（必须最新）
- 已完成 Codex Desktop watcher 的 Stage 1 代码迁移：就绪判断现在要求透明 manifest、shim、real CLI、原版 app.asar 与全部 helper；构建命令只调用 Stage 1 + StableOnly，并把完整 stdout/stderr 写入 watcher 日志；专项测试尚待执行，当前官方 Desktop不会被 watcher 重启。
