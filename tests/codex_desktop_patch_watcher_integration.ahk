...
; ============================================
; Codex Desktop Stage 5 watcher 真实构建入口冒烟测试
; --------------------------------------------
; 该测试不会启动或关闭 Desktop UI。当前版本 Stage 5 已存在时，构建器只做安全跳过，
...
result := CodexDesktopPatchWatcherBuildStage5(version, false)
if !result["ok"] {
    throw Error("真实 Stage 5 watcher 构建入口失败：" result["message"] "`n" result["detail"])
}
if !CodexDesktopPatchWatcherIsStage5Ready(version) {
    throw Error("构建入口返回成功后 Stage 5 仍未就绪：" version)
}
if CodexDesktopPatchWatcherReadLastBuiltVersion() != version {
    throw Error("成功构建后 watcher 状态版本没有更新：" version)
}
if !InStr(result["detail"], "stage5") {
    throw Error("构建输出没有被 AHK 完整捕获")
}

FileAppend("PASS: Stage 5 watcher integration version=" version "`n", "*")
