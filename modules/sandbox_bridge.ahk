...
; - 仅在微信/QQ 窗口执行粘贴/清理触发，避免误操作到其他软件。
; ============================================
...
global g_SandboxBridgeState := {                              ; 用对象保存一次任务全量状态
    ...
    lastCleanupTapTick: 0                                     ; 多文件模式下用于判定“双击清理”的时间戳
}
...
SandboxBridgeStageFromSelection() {
    ...
    if g_SandboxBridgeState.singleFileMode {
        ...
        Toast("✅ 中转完成（多项）。发送完成后在微信/QQ内快速按两次 Ctrl+Alt+V 清理")
    }
...
; 逻辑：
; - 单文件模式：自动设置文件剪贴板并发送 Ctrl+V，然后延时自动清理。
; - 多文件模式：第一次按提示，2.5 秒内第二次按执行清理。
SandboxBridgePasteOrCleanup() {
    ...
    targetKey := SandboxBridgeDetectActiveTarget()  ; 判定当前窗口属于微信还是 QQ
    if (targetKey = "") {
    ...
    if g_SandboxBridgeState.singleFileMode {
        ...
        Toast("📎 已触发粘贴，约 90 秒后自动清理中转文件")
        return true
    }

    ; 多文件/目录模式：第一次按键仅记时间并提示，第二次（短时间内）才清理。
    nowTick := A_TickCount
    if (nowTick - g_SandboxBridgeState.lastCleanupTapTick <= g_SandboxBridgeDoubleTapMs) {
        SandboxBridgeCleanupStagedFiles(true)  ; 第二次触发：直接清理
        return true
    }

    g_SandboxBridgeState.lastCleanupTapTick := nowTick
    Toast("🧹 多文件模式：你手动发送完成后，请在 2.5 秒内再按一次 Ctrl+Alt+V 清理")
    return true
}
